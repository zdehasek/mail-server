...
When `ENABLE_SSH_HARDENING=true`, the installer writes `/etc/ssh/sshd_config.d/99-mailserver-hardening.conf` with root login and password login disabled. Set `SSH_ALLOW_USERS` in `~/.mail-server/config.env` to the SSH user that should remain allowed, for example:
