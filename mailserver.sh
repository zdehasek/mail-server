...
wizard_record_color_code() {
  ...
  if [[ "$line" =~ ^[[:alnum:]_.-]+\.\ (MX|A|AAAA|TXT|CAA|CNAME|TLSA)\  ]]; then
    ...
    printf '%s\n' "1;36"
  elif [[ "$line" == "Publish these DNS records:" || "$line" == "DKIM record:" || "$line" == "DKIM record generated by OpenDKIM:" || "$line" == "DKIM record generated locally:" || "$line" == "Optional TLS policy DNS records:" || "$line" == "Provider PTR/rDNS must be:" ]]; then
    printf '%s\n' "1"
